...
public class YMarking {
    ...
    public YSetOfMarkings reachableInOneStep(YTask task, YTask orJoin) {
        ...
        if (_locations.contains(task)) {
            org.yawlfoundation.yawl.elements.state.YMarking aMarking = new org.yawlfoundation.yawl.elements.state.YMarking(_locations);
        ...
        for (org.yawlfoundation.yawl.elements.state.YMarking halfbakedMarking : halfBakedSet.getMarkings()) {
            halfbakedMarking._locations.removeAll(task.getRemoveSet());
        }

        Set<org.yawlfoundation.yawl.elements.state.YMarking> iterableHalfBakedSet = halfBakedSet.getMarkings();
        ...
        switch (task.getSplitType()) {
            case YTask._AND:
            case YTask._OR: {
                for (org.yawlfoundation.yawl.elements.state.YMarking marking : iterableHalfBakedSet) {
            ...
            case YTask._XOR: {
                for (org.yawlfoundation.yawl.elements.state.YMarking halfbakedMarking : iterableHalfBakedSet) {
                    for (YExternalNetElement element : postset) {
                        org.yawlfoundation.yawl.elements.state.YMarking aFinalMarking = new org.yawlfoundation.yawl.elements.state.YMarking(halfbakedMarking.getLocations());
        ...
        switch (joinType) {
            case YTask._AND:
            {
                if (!nonOrJoinEnabled(task)) {
                    ...
                    org.yawlfoundation.yawl.elements.state.YMarking returnedMarking = new org.yawlfoundation.yawl.elements.state.YMarking(_locations);
    ...
    public boolean equals(Object marking) {
        if (!(marking instanceof org.yawlfoundation.yawl.elements.state.YMarking)) {
            return false;
        }
        List otherMarkingsLocations = new Vector(((org.yawlfoundation.yawl.elements.state.YMarking) marking).getLocations());
    ...
    public boolean strictlyGreaterThanOrEqualWithSupports(org.yawlfoundation.yawl.elements.state.YMarking marking) {
    ...
    //moe - ResetAnalyser
    public boolean isBiggerThanOrEqual(org.yawlfoundation.yawl.elements.state.YMarking marking) {
        return this.isBiggerThan(marking) || this.equivalentTo(marking);
    }


    //moe - ResetAnalyser
    public boolean isBiggerThan(org.yawlfoundation.yawl.elements.state.YMarking marking) {
        List otherMarkingsLocations = new Vector(marking.getLocations());
        List myLocations = new Vector(_locations);
    ...
    public boolean strictlyLessThanWithSupports(org.yawlfoundation.yawl.elements.state.YMarking marking) {
        List otherMarkingsLocations = new Vector(marking.getLocations());
        List myLocations = new Vector(_locations);
        if (!(myLocations.containsAll(otherMarkingsLocations)
                && otherMarkingsLocations.containsAll(myLocations))) {
    ...
    public boolean isBiggerEnablingMarkingThan(org.yawlfoundation.yawl.elements.state.YMarking marking, YTask orJoin) {
    ...
    public boolean equivalentTo(org.yawlfoundation.yawl.elements.state.YMarking marking) {
